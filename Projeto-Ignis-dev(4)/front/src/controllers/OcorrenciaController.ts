...
class OcorrenciaController {
  ...
  public async Filtrar_cerrado(req: Request, res: Response): Promise<void> {
    try {
      const r: ResultadoQuery[] = await query(`
    SELECT
    ST_Y(a.geom) AS latitude,         -- Latitude extraída do campo geom
    ST_X(a.geom) AS longitude,        -- Longitude extraída do campo geom
    e.estado AS estado,               -- Nome do estado
    b.bioma AS bioma,                 -- Nome do bioma (Cerrado)
    a.risco AS risco_fogo,            -- Risco de fogo
    a.data_pas AS data,               -- Data do registro
    a.frp                             -- Potência radiativa do fogo
FROM Area_Queimada a
JOIN Estados e ON a.estado_id = e.id_estado
JOIN Bioma b ON a.bioma_id = b.id
WHERE b.bioma = 'Cerrado';
